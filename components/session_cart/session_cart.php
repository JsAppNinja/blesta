...
class SessionCart {
	...
	/**
	 * @var array An array of callbacks in key/value pairs where each key is the name of the method to invoke the callback on
	 */
	...
	public function setCallback($action, $callback) {
		$this->Events->register("SessionCart." . $this->cart_name . "." . $action, $callback);
	}
	...
	public function removeItems(array $indexes) {
		...
		foreach ($indexes as $index) {
			if (isset($cart['items'][$index])) {
				...
				$event = new EventObject("SessionCart." . $this->cart_name . ".removeItem", array('item' => $item, 'index' => $index));
				$this->Events->trigger($event);
			}
		}
	}
	...
	public function prequeueItem($item) {
		$event = new EventObject("SessionCart." . $this->cart_name . ".prequeueItem", array('item' => $item));
		$this->Events->trigger($event);
		
		$return_item = $event->getReturnVal();
	...
	public function dequeue($index = 0) {
		...
		if (!isset($cart['queue']) || !array_key_exists($index, $cart['queue']))
			return null;
