...
class Converge extends MerchantGateway implements MerchantCc
{
    ...
    {
        return array("merchant_id", "user_id", "pin");
    }
     ...
     * @return boolean True if the gateway expects the offset methods to be called for
    ...
    public function processCc(array $card_info, $amount, array $invoice_amounts = null)
    {
        ...
        if ($invoice_amounts) {
            $actionInfo['ssl_invoice_number'] = $this->ifSet($invoice_amounts[0]['invoice_id']);
        }
        ...
        return array(
            ...
            'transaction_id' => $this->ifSet($response['ssl_txn_id']),
            'message' => $this->ifSet($response['ssl_result_message'])
        );
    }
