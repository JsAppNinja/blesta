...
class Skrillapi {
     ...
     * @var The payment URL
     */
    private $url = "https://www.moneybookers.com/app/payment.pl";
    /**
     * @var The refund URL
     */
    private $refund_url = "https://www.moneybookers.com/app/refund.pl";
	...
	public function prepareRequest($args = false, $request_type = null, $sid = null){	
		...
		if($request_type == "refund"){
			...
			$url = $this->refund_url;
			$args = array_merge($params, (is_array($args) ? $args : array()));
		}
	...
	public function validateResponse($fields) {
        ...
        // Check the signature
        $signature = md5($req_fields['merchant_id'] . $req_fields['mb_transaction_id'] . strtoupper(md5($this->secret_word)) . $req_fields['mb_amount'] . $req_fields['mb_currency'] . $req_fields['status']);
