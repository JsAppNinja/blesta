	...
	private function drawPayments() {
		if (!empty($this->meta['display_payments']) && $this->meta['display_payments'] == "true") {
			...
			for ($i=0; $i<count($this->invoice->applied_transactions); $i++) {
				...
				if ($this->invoice->applied_transactions[$i]->type == "other" && $this->invoice->applied_transactions[$i]->gateway_type == "nonmerchant")
			...
			$data = array(array('blank'=>"",'label'=>Language::_("DefaultInvoice.balance_heading", true),'price'=>$this->CurrencyFormat->format($this->invoice->due, $this->invoice->currency)));
