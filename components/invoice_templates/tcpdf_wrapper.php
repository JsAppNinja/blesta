<?php
Loader::load(VENDORDIR . "tcpdf" . DS . "tcpdf.php");
/**
			...
			foreach ($row as $col => $text) {
				...
				if (isset($cell_options['font'])) {
					if (is_array($cell_options['font']))
						...
						$this->SetFont($cell_options['font'], isset($cell_options['font_style']) ? $cell_options['font_style'] : null);
				}
				...
				if ($cell_options['type'] == 'cell')
					...
					if ($end_row)
						$lines = $this->MultiCell($w, $h, $text, $border, $text_align, $fill, 1, $this->GetX(), $y_start);
					else
						$lines = $this->MultiCell($w, $h, $text, $border, $text_align, $fill, 2, $this->GetX(), $y_start);
				}
