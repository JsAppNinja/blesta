...
class Upgrade3_6_0B1 extends UpgradeUtil {
	...
	public function addTransactionType($undo = false) {
		...
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `transaction_types` ADD `type` ENUM('debit','credit') CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT 'debit' AFTER `name`;");
			
			// Set the included in house credit type to credit
			$this->Record->where("name", "=", "in_house_credit")
				->update("transaction_types", array('type' => "credit"));
		}
	}
	...
	public function increaseContactCompanyLength($undo = false) {
		if ($undo) {
			$this->Record->query("ALTER TABLE `contacts` CHANGE `company` `company` VARCHAR(64) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL;");
		}
		else {
			$this->Record->query("ALTER TABLE `contacts` CHANGE `company` `company` VARCHAR(128) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL;");
		}
	}
}
