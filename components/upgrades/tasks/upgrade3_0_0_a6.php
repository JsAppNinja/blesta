...
class Upgrade3_0_0A6 extends UpgradeUtil {
	...
	private function updatePluginEvents($undo = false) {
		...
		if ($undo) {
			$this->Record->query(
				"ALTER TABLE `plugin_events` CHANGE `event` `event` VARCHAR( 32 ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL;"
			);
		}
		else {
			$this->Record->query(
				"ALTER TABLE `plugin_events` CHANGE `event` `event` VARCHAR( 128 ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL;"
			);
		}
	}
}
