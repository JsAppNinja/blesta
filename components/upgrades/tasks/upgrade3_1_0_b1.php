...
class Upgrade3_1_0B1 extends UpgradeUtil {
	...
	private function updatePackages($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `packages` ADD `single_term` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '0' AFTER `taxable` ;");
		}
	}
	...
	private function addPackageOptions($undo = false) {
		if ($undo) {
			try {
				...
				$fields = array("package_pricing.*", "pricings.term", "pricings.period", "pricings.price",
				...
				$this->Record->
					...
					setField("term", array('type' => "smallint", 'size' => 5, 'unsigned' => true, 'default' => 1))->
					setField("period", array('type' => "enum", 'size' => "'day','week','month','year','onetime'", 'default' => "month"))->
					setField("price", array('type' => "decimal", 'size' => "12,4", 'default' => "0.0000"))->
					setField("setup_fee", array('type' => "decimal", 'size' => "12,4", 'default' => "0.0000"))->
					setField("cancel_fee", array('type' => "decimal", 'size' => "12,4", 'default' => "0.0000"))->
					setField("currency", array('type' => "char", 'size' => 3, 'default' => "USD"))->
					setKey(array("package_id"), "index")->
			...
			$this->Record->
				setField("package_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("option_group_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setKey(array("package_id", "option_group_id"), "primary")->
			...
			$this->Record->
				...
				setField("label", array('type' => "varchar", 'size' => 128))->
				setField("name", array('type' => "varchar", 'size' => 128))->
				setField("type", array('type' => "enum", 'size' => "'checkbox','radio','select','quantity'", 'default' => "select"))->
			...
			$this->Record->
				setField("id", array('type' => "int", 'size' => 10, 'unsigned' => true, 'auto_increment' => true))->
				setField("option_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("name", array('type' => "varchar", 'size' => 128))->
				setField("value", array('type' => "varchar", 'size' => 255, 'is_null' => true, 'default' => null))->
				setField("order", array('type' => "smallint", 'size' => 5, 'unsigned' => true))->
				setField("min", array('type' => "int", 'size' => 10, 'unsigned' => true, 'is_null' => true, 'default' => null))->
				setField("max", array('type' => "int", 'size' => 10, 'unsigned' => true, 'is_null' => true, 'default' => null))->
				setField("step", array('type' => "int", 'size' => 10, 'unsigned' => true, 'is_null' => true, 'default' => null))->
				setKey(array("option_id"), "index")->
				setKey(array("id"), "primary")->
			...
			$this->Record->
				setField("id", array('type' => "int", 'size' => 10, 'unsigned' => true, 'auto_increment' => true))->
				setField("option_value_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("pricing_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setKey(array("option_value_id", "pricing_id"), "unique")->
				setKey(array("id"), "primary")->
			...
			$this->Record->
				...
				setField("option_group_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("order", array('type' => "smallint", 'size' => 5, 'unsigned' => true))->
				setKey(array("option_id", "option_group_id"), "primary")->
			...
			$this->Record->
				...
				setField("option_pricing_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("qty", array('type' => "int", 'size' => 10, 'unsigned' => true, 'default' => 1))->
				setKey(array("service_id"), "index")->
				setKey(array("option_pricing_id"), "index")->
				setKey(array("id"), "primary")->
			...
			foreach ($pricing as $price) {
				...
				$package_pricing_ids[] = array('id' => $price->id, 'package_id' => $price->package_id, 'pricing_id' => $this->Record->lastInsertId());
			}
			...
			$this->Record->
				...
				setField("package_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("pricing_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setKey(array("package_id", "pricing_id"), "unique")->
				setKey(array("id"), "primary")->
				create("package_pricing", true);
		...
		if ($undo) {
			...
			foreach ($staff_groups as $staff_group) {
				$staff_group_access[$staff_group->id] = array(
					'admin_system_general' => $this->Permissions->authorized("staff_group_" . $staff_group->id, "admin_system_general", "basic", "staff", $staff_group->company_id),
					'admin_company_billing' => $this->Permissions->authorized("staff_group_" . $staff_group->id, "admin_company_billing", "customization", "staff", $staff_group->company_id),
					'admin_company_general' => $this->Permissions->authorized("staff_group_" . $staff_group->id, "admin_company_general", "localization", "staff", $staff_group->company_id),
					'admin_packages' => $this->Permissions->authorized("staff_group_" . $staff_group->id, "admin_packages", "groups", "staff", $staff_group->company_id)
				);
			}
	...
	private function addEmailAttachments($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `emails` ADD `include_attachments` TINYINT( 1 ) NOT NULL DEFAULT '1' AFTER `email_signature_id`;");
		}
	}
	...
	private function addServiceUnsuspensionEmail($undo = false) {
		...
		Loader::loadModels($this, array("Companies", "Emails", "Languages"));
		
		// Add the service unsuspension email template group
		$this->Record->query("INSERT INTO `email_groups` (`id` , `action` , `type` , `plugin_dir` , `tags`) VALUES (NULL , 'service_unsuspension', 'client', NULL , '{contact.first_name},{contact.last_name},{package.name}');");
...
<p>Your service, {package.name} has been unsuspended.</p>",
	...
	private function updateServiceSuspensionErrorEmail($undo = false) {
		...
		$vars = array(
			'tags' => "{staff.first_name},{staff.last_name},{client.id_code},{client.first_name},{client.last_name},{client.email},{service.name},{package.name}"
		);
		...
		$emails = $this->Record->select(array("emails.*"))->from("emails")->
...
<p>
	...
	private function addServiceCreationErrorEmail($undo = false) {
		...
		$this->Record->query("INSERT INTO `email_groups` (`id` , `action` , `type` , `notice_type` , `plugin_dir` , `tags`) VALUES (NULL , 'service_creation_error', 'staff', 'to', NULL , '{staff.first_name},{staff.last_name},{client.id_code},{client.first_name},{client.last_name},{client.email},{service.name},{package.name}');");
		$email_group_id = $this->Record->lastInsertId();
...
<p>
	...
	private function addPackageGroupSort($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `package_group` ADD `order` SMALLINT( 5 ) NOT NULL DEFAULT '0' AFTER `package_group_id`;");
		}
	}
	...
	private function updateAutodebitSettings($undo = false) {
		...
		if ($undo) {
			// System setting
			$this->Record->query("UPDATE `settings` SET `key`='autodebit_backoff', `value`='false' WHERE `key`='autodebit_attempts' AND `value`='1';");
			$this->Record->query("UPDATE `settings` SET `key`='autodebit_backoff', `value`='true' WHERE `key`='autodebit_attempts' AND `value`='3';");
			// Company settings
			$this->Record->query("UPDATE `company_settings` SET `key`='autodebit_backoff', `value`='false' WHERE `key`='autodebit_attempts' AND `value`='1';");
			$this->Record->query("UPDATE `company_settings` SET `key`='autodebit_backoff', `value`='true' WHERE `key`='autodebit_attempts' AND `value`='3';");
			// Client group settings
			$this->Record->query("UPDATE `client_group_settings` SET `key`='autodebit_backoff', `value`='false' WHERE `key`='autodebit_attempts' AND `value`='1';");
			$this->Record->query("UPDATE `client_group_settings` SET `key`='autodebit_backoff', `value`='true' WHERE `key`='autodebit_attempts' AND `value`='3';");
			
			$this->Record->query("ALTER TABLE `client_account` DROP `failed_count`;");
		}
		else {
			// System setting
			$this->Record->query("UPDATE `settings` SET `key`='autodebit_attempts', `value`='1' WHERE `key`='autodebit_backoff' AND `value`='false';");
			$this->Record->query("UPDATE `settings` SET `key`='autodebit_attempts', `value`='3' WHERE `key`='autodebit_backoff' AND `value`='true';");
			// Company settings
			$this->Record->query("UPDATE `company_settings` SET `key`='autodebit_attempts', `value`='1' WHERE `key`='autodebit_backoff' AND `value`='false';");
			$this->Record->query("UPDATE `company_settings` SET `key`='autodebit_attempts', `value`='3' WHERE `key`='autodebit_backoff' AND `value`='true';");
			// Client group settings
			$this->Record->query("UPDATE `client_group_settings` SET `key`='autodebit_attempts', `value`='1' WHERE `key`='autodebit_backoff' AND `value`='false';");
			$this->Record->query("UPDATE `client_group_settings` SET `key`='autodebit_attempts', `value`='3' WHERE `key`='autodebit_backoff' AND `value`='true';");
			
			$this->Record->query("ALTER TABLE `client_account` ADD `failed_count` SMALLINT( 5 ) NOT NULL DEFAULT '0' AFTER `type`;");
			
			// Allow staff_id to be null (allow system to create notes)
			$this->Record->query("ALTER TABLE `client_notes` CHANGE `staff_id` `staff_id` INT( 10 ) UNSIGNED NULL DEFAULT NULL;");
		}
	}
	...
	private function updateEmailGroupFields($undo = false) {
		if ($undo) {
			...
			// Add the column
			$this->Record->query("ALTER TABLE `email_groups` ADD `notice_type` ENUM( 'bcc', 'to' ) NULL DEFAULT NULL AFTER `type` ;");
			
			// Update all core client email groups to BCC
			$this->Record->query("UPDATE `email_groups` SET `notice_type` = 'bcc' WHERE `type` = 'client' AND `plugin_dir` IS NULL ;");
			
			// Update all core staff email groups to TO, except for the password reset template
			$this->Record->query("UPDATE `email_groups` SET `notice_type` = 'to' WHERE `type` = 'staff' AND `action` != 'staff_reset_password' AND `plugin_dir` IS NULL ;");
		}
	}
