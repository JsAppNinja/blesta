...
class Upgrade3_4_0B1 extends UpgradeUtil {
	...
	private function addNonmerchantPaymentEmail($undo = false) {
		...
		$this->Record->query("INSERT INTO `email_groups` (`id`, `action`, `type`, `notice_type`, `plugin_dir`, `tags`) VALUES (NULL, 'payment_nonmerchant_approved', 'client', 'bcc', NULL, '{contact.first_name},{contact.last_name},{transaction.amount},{transaction.currency},{transaction.transaction_id},{date_added}');");
...
Thank you for your business!",
	...
	private function addCompanySettings($undo = false) {
		...
		$company_settings = array('send_payment_notices' => "true", 'show_client_tax_id' => "true");
		...
		foreach ($companies as $company) {
			foreach ($company_settings as $setting => $default_value) {
				if ($undo) {
					$this->Record->from("company_settings")->where("key", "=", $setting)->delete(array("company_settings.*"));
				}
				else {
					$this->Record->insert("company_settings", array('key' => $setting, 'company_id' => $company->id, 'value' => $default_value));
				}
			}
		}
	}
	...
	private function addDisplayPaymentsSetting($undo = false) {
		...
		foreach ($companies as $company) {
			if ($undo) {
				$this->Record->from("company_settings")->where("key", "=", "inv_display_payments")->delete(array("company_settings.*"));
			}
			else {
				$this->Record->insert("company_settings", array('key' => "inv_display_payments", 'company_id' => $company->id, 'value' => "true"));
			}
		}
	}
	...
	private function updatePlugins($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `plugins` ADD `enabled` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '1' AFTER `version`;");
		...
		foreach ($companies as $company) {
			...
			foreach ($languages as $language) {
				...
				if ($welcome_email) {
					...
					$vars = array(
						'text' => str_replace("Password: {password}", "Password: (Use the password you signed up with)", $welcome_email->text),
						'html' => str_replace("Password: {password}<br />", "Password: (Use the password you signed up with)<br />", $welcome_email->html)
					);
					
					if ($welcome_email->text != $vars['text'] || $welcome_email->html != $vars['html'])
						$this->Record->where("id", "=", $welcome_email->id)->update("emails", $vars, array("text", "html"));
				}
			}
		}
	}
	...
	private function addContactPermissions($undo = false) {
		if ($undo) {
			...
			$this->Record->
				setField("contact_id", array('type' => "int", 'size' => 10))->
				setField("area", array('type' => "varchar", 'size' => 255))->
				setKey(array("contact_id", "area"), "primary")->
	...
	private function addSessionIp($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `staff_groups` ADD `session_lock` TINYINT( 1 ) NOT NULL DEFAULT '1' AFTER `name`;");
