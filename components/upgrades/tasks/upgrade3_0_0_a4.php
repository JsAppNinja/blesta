...
class Upgrade3_0_0A4 extends UpgradeUtil {
	...
	private function alterCustomFields($undo = false) {
		if ($undo) {
			$this->Record->query(
				"ALTER TABLE `client_fields` CHANGE `show_client` `show_client` TINYINT( 1 ) SIGNED NOT NULL DEFAULT '0';
				ALTER TABLE `client_fields` DROP `read_only`;"
			);
		}
		else {
			$this->Record->query(
				"ALTER TABLE `client_fields` CHANGE `show_client` `show_client` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '0';
	...
	private function updateInvoiceTotals($undo = false) {
		if ($undo) {
			...
			$invoices = $this->Record->select(array("invoices.id"))->from("invoices")->where("subtotal", "=", "0.0000")->fetchAll();
			
			foreach ($invoices as $invoice) {
				$subtotal = $this->Invoices->getSubtotal($invoice->id);
				$total = $this->Invoices->getTotal($invoice->id);
	 ...
	 * UPdate emails
	 *
	 * @param boolean $unit True to undo the change, false to perform the change
	 */
...
If you have any questions about your invoice, please let us know!</p>"
			...
			$this->Record->where("emails.email_group_id", "=", 9)->where("emails.lang", "=", "en_us")->update("emails", $fields);
		}
	}
}
?>