...
class Upgrade3_5_1 extends UpgradeUtil {
	...
	public function addCurrencyPrecision($undo = false) {
		...
		if ($undo) {
			...
			// Add precision
			$statement = $this->Record->query("ALTER TABLE `currencies` ADD `precision` TINYINT(1) UNSIGNED NOT NULL DEFAULT '4' AFTER `format`;");
			...
			foreach ($currencies as $currency => $precision) {
				$this->Record->where("code", "=", $currency)->
					update("currencies", array('precision' => $precision));
			}
		}
	}
}
