...
class Upgrade3_3_0B1 extends UpgradeUtil {
		...
		if ($undo) {
			...
			foreach ($companies as $company) {
				$this->Record->from("company_settings")->
					...
					where("key", "in", array("client_change_service_term", "inv_type", "inv_proforma_format", "inv_proforma_start"))->
					delete();
			}
		}
		else {
    ...
    private function inheritSettings($undo = false) {
        if ($undo) {
            ...
            $this->Record->query("UPDATE `company_settings` SET `inherit` = '0' WHERE `company_settings`.`key` = 'tax_id';");
        }
    }
	...
	private function updateInvoices($undo = false) {
		...
		if ($undo) {
			$this->Record->query("ALTER TABLE `invoices` CHANGE `status` `status` ENUM( 'active', 'draft', 'void' ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT 'active';");
		}
		else {
			$this->Record->query("ALTER TABLE `invoices` CHANGE `status` `status` ENUM( 'active', 'proforma', 'draft', 'void' ) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT 'active';");
		}
	}
	...
	private function priceOverride($undo = false) {
		...
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `services` ADD `override_price` DECIMAL( 12, 4 ) NULL DEFAULT NULL AFTER `qty`;");
			$this->Record->query("ALTER TABLE `services` ADD `override_currency` VARCHAR( 3 ) NULL DEFAULT NULL AFTER `override_price`;");
		}
	}
    ...
    private function addEmailTemplates($undo = false) {
		...
		$this->Record->query("INSERT INTO `email_groups` (`id`, `action`, `type`, `notice_type`, `plugin_dir`, `tags`) VALUES (NULL, 'report_tax_liability', 'staff', 'to', NULL, '{staff.first_name},{staff.last_name},{company.name}');");
...
<p>A Tax Liability Report has been generated for {company.name} and is attached to this email as a CSV file.</p>",
	...
	private function addModuleClientMeta($undo = false) {
		if ($undo) {
			...
			$this->Record->
				setField("module_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("module_row_id", array('type' => "int", 'size' => 10, 'unsigned' => true, 'default' => 0))->
				setField("client_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("key", array('type' => "varchar", 'size' => 32))->
				setField("value", array('type' => "text", 'is_null' => true, 'default' => null))->
				setField("serialized", array('type' => "tinyint", 'size' => 1, 'default' => 0))->
				setField("encrypted", array('type' => "tinyint", 'size' => 1, 'default' => 0))->
				setKey(array("client_id", "key", "module_id", "module_row_id"), "primary")->
