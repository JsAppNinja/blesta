...
class Upgrade3_5_0B1 extends UpgradeUtil {
	...
	private function addPackageOptionPermissions($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `package_options` ADD `addable` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '1' AFTER `type`;");
			$this->Record->query("ALTER TABLE `package_options` ADD `editable` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '1' AFTER `addable`;");
		}
	}
	...
	private function addInvoiceDateDueSettings($undo = false) {
		if ($undo) {
			...
			foreach ($companies as $company) {
				$this->Record->query("INSERT INTO `company_settings` (`key`, `company_id`, `value`) VALUES ('inv_display_due_date_draft', ?, 'true');", $company->id);
				$this->Record->query("INSERT INTO `company_settings` (`key`, `company_id`, `value`) VALUES ('inv_display_due_date_proforma', ?, 'true');", $company->id);
				$this->Record->query("INSERT INTO `company_settings` (`key`, `company_id`, `value`) VALUES ('inv_display_due_date_inv', ?, 'true');", $company->id);
			}
		}
	}
	
	/**
	 * Adds a new coupon option to identify whether or not the coupon applies to package options in addition to the package itself
	 */
	private function addCouponConfigOptionSetting($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `coupons` ADD `apply_package_options` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' ;");
		}
	}
		...
		if ($undo) {
			...
			foreach ($staff_groups as $staff_group) {
				$staff_group_access[$staff_group->id] = array(
					'admin_reports' => $this->Permissions->authorized("staff_group_" . $staff_group->id, "admin_billing", "reports", "staff", $staff_group->company_id)
				);
			}
