...
class Upgrade3_5_0B2 extends UpgradeUtil {
	...
	private function addReports($undo = false) {
		if ($undo) {
			...
			$this->Record->
				...
				setField("query", array('type' => "mediumtext"))->
				setField("date_created", array('type' => "datetime"))->
				setKey(array("company_id", "date_created"), "index")->
				setKey(array("id"), "primary")->
				create("reports", true);

			$this->Record->
				setField("id", array('type' => "int", 'size' => 10, 'unsigned' => true, 'auto_increment' => true))->
				setField("report_id", array('type' => "int", 'size' => 10, 'unsigned' => true))->
				setField("name", array('type' => "varchar", 'size' => 128))->
				setField("label", array('type' => "varchar", 'size' => 255))->
				setField("type", array('type' => "enum", 'size' => "'text','date','select'"))->
				setField("values", array('type' => "text", 'default' => null, 'is_null' => true))->
				setField("regex", array('type' => "text", 'default' => null, 'is_null' => true))->
				setKey(array("report_id"), "index")->
				setKey(array("id"), "primary")->
	...
	private function addPackageGroupUpgradeSetting($undo = false) {
		if ($undo) {
			...
			$this->Record->query("ALTER TABLE `package_groups` ADD `allow_upgrades` TINYINT(1) UNSIGNED NOT NULL DEFAULT '1' ;");
		}
	}
		...
		if ($undo) {
			...
			foreach ($staff_groups as $staff_group) {
				$staff_group_access[$staff_group->id] = array(
					'admin_reports_customize' => $this->Permissions->authorized("staff_group_" . $staff_group->id, "admin_system_backup", "*", "staff", $staff_group->company_id)
				);
			}
	...
	private function addServiceUpgrades($undo = false) {
		...
		if ($undo) {
			...
			$this->Record->from("cron_tasks")->
				...
				delete(array("cron_tasks.*", "cron_task_runs.*"));
			...
			$this->Record->
				...
				setField("status", array('type' => "enum", 'size' => "'pending','completed','canceled','error'", 'default' => "pending"))->
				setField("data", array('type' => "mediumtext"))->
				setField("date_added", array('type' => "datetime"))->
				setField("date_status", array('type' => "datetime"))->
				setKey(array("service_id"), "index")->
				setKey(array("invoice_id"), "unique")->
				setKey(array("status", "date_status"), "index")->
				setKey(array("id"), "primary")->
			...
			foreach ($companies as $company) {
				...
				$this->Record->query("INSERT INTO `company_settings` (`key`, `company_id`, `value`) VALUES ('process_paid_service_changes', ?, 'false');", $company->id);
				$this->Record->query("INSERT INTO `company_settings` (`key`, `company_id`, `value`) VALUES ('cancel_service_changes_days', ?, '7');", $company->id);
			}
		}
	}
}
