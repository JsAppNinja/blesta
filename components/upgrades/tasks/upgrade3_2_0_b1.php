...
class Upgrade3_2_0B1 extends UpgradeUtil {
	 ...
	 *
	 * @param boolean $undo True to undo the change, or false to perform the change
	 */
	private function updateClientThemes($undo = false) {
		$themes = $this->Record->select()->from("themes")->where("type", "=", "client")->getStatement();
		$default_themes = array("Blesta Blue", "Minegrass", "Cloudy Day", "Vintage", "Clean");
		$default_colors = array("theme_header_bg_color_top", "theme_header_bg_color_bottom", "theme_page_title_background_color_top",
			"theme_page_title_background_color_bottom", "theme_page_title_text_color", "theme_navigation_background_color_top",
			"theme_navigation_background_color_bottom", "theme_navigation_text_color", "theme_navigation_text_active_color",
			"theme_page_background_color", "theme_panel_header_background_color_top", "theme_panel_header_background_color_bottom",
			"theme_panel_header_text_color", "theme_link_color", "theme_link_settings_color", "theme_highlight_hover_color"
		);
			...
			foreach ($themes as $theme) {
				if ($data && isset($data['colors'])) {
					...
					if ($data && isset($data['colors'])) {
						...
						unset($new_data['colors']['theme_panel_header_background_color_top'],
							$new_data['colors']['theme_panel_header_background_color_bottom'],
							$new_data['colors']['theme_panel_header_text_color'], $new_data['colors']['theme_highlight_hover_color']
						);
						
						// Update the theme
						$fields = array("data");
						
						$vars = array('data' => base64_encode(serialize($new_data)));
			...
			foreach ($themes as $theme) {
				...
				if ($data && isset($data['colors'])) {
					...
					foreach ($default_colors as $name) {
						if (!array_key_exists($name, $data['colors']))
							$data['colors'][$name] = "";
					}
					...
					$new_data = array(
						'colors' => array(
							'theme_header_bg_color_top' => $data['colors']['theme_header_bg_color_top'],
							'theme_header_bg_color_bottom' => $data['colors']['theme_header_bg_color_bottom'],
							'theme_page_title_background_color_top' => $data['colors']['theme_page_title_background_color_top'],
							'theme_page_title_background_color_bottom' => $data['colors']['theme_page_title_background_color_bottom'],
							'theme_page_title_text_color' => $data['colors']['theme_page_title_text_color'],
							'theme_navigation_background_color_top' => $data['colors']['theme_navigation_background_color_top'],
							'theme_navigation_background_color_bottom' => $data['colors']['theme_navigation_background_color_bottom'],
							'theme_navigation_text_color' => $data['colors']['theme_navigation_text_color'],
							'theme_navigation_text_active_color' => $data['colors']['theme_navigation_text_active_color'],
							'theme_page_background_color' => $data['colors']['theme_page_background_color'],
							'theme_panel_header_background_color_top' => $theme_colors[$theme->name]['theme_panel_header_background_color_top'],
							'theme_panel_header_background_color_bottom' => $theme_colors[$theme->name]['theme_panel_header_background_color_bottom'],
							'theme_panel_header_text_color' => $theme_colors[$theme->name]['theme_panel_header_text_color'],
							'theme_link_color' => $data['colors']['theme_link_color'],
							'theme_link_settings_color' => $data['colors']['theme_link_settings_color'],
							'theme_highlight_hover_color' => $theme_colors[$theme->name]['theme_highlight_hover_color']
						),
						'logo_url' => (isset($data['logo_url']) ? $data['logo_url'] : "")
					);
	...
	public function addClientAddonSetting($undo = false) {
		if ($undo) {
			...
			// Add the new setting. Default to 'false' to maintain current behavior for existing installations only, assuming they have clients
			...
			foreach ($client_groups as $group) {
				$this->Record->insert("client_group_settings", array('key' => "client_create_addons", 'value' => $setting_value, 'client_group_id' => $group->id));
			}
			
			// Add to company settings
			$companies = $this->Record->select()->from("companies")->getStatement();
			foreach ($companies as $company) {
				$this->Record->insert("company_settings", array('key' => "client_create_addons", 'value' => $setting_value, 'company_id' => $company->id));
			}
		}
	}
	...
	public function updateServiceSuspensionEmails($undo = false) {
		...
		$actions = array("service_suspension", "service_unsuspension");
		$email_groups = array();
		...
		if ($undo) {
			...
			// Update the email templates, but only for new installations, assuming new installations do not have clients
			$clients = $this->Record->select()->from("clients")->numResults();
			...
			if ($clients == 0) {
				...
				foreach ($email_groups as $email_group_id) {
					$this->Record->where("id", "=", $email_group_id)->update("email_groups", array('tags' => $tags));
				}
			}
		}
		...
		if (isset($fields)) {
			foreach ($email_groups as $email_group_id) {
				$emails = $this->Record->select()->from("emails")->where("email_group_id", "=", $email_group_id)->fetchAll();
