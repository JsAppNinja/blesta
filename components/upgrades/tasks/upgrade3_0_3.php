...
class Upgrade3_0_3 extends UpgradeUtil {
	...
	private function fixServiceSuspensionTemplate($undo = false) {
		...
		$vars = array('tags' => "{contact.first_name},{contact.last_name},{package.name}");
		...
		$email_group = $this->Record->select()->from("email_groups")->where("action", "=", "service_suspension")->fetch();
