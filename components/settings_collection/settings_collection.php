...
class SettingsCollection {
	...
	public function fetchSettings(Companies $companies=null, $company_id=null, $ignore_inheritence=false) {
		...
		return $this->ArrayHelper->numericToKey($companies->getSettings($company_id, $ignore_inheritence), "key", "value");
	}
	 ...
	 */
	public function fetchClientGroupSettings($client_group_id, ClientGroups $settings=null, $ignore_inheritence=false) {
		$settings = $this->loadIfNotSet($settings, "ClientGroups");
		return $this->ArrayHelper->numericToKey($settings->getSettings($client_group_id, $ignore_inheritence), "key", "value");
	}
	...
	private function loadIfNotSet($obj, $model) {
		if (!$obj) {
			if (!isset($this->$model))
				Loader::loadModels($this, array($model));
			return $this->$model;
		}
		return $obj;
	}
