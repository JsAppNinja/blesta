...
class PleskResellerAccounts extends PleskPacket {
	...
	protected function buildContainer() {
		$this->insert(array('reseller' => null));
	 ...
	 * 	- zipcode The reseller's zip/postal code (optional, should only be used for US citizens; 0 to 10 characters in length)
	...
	public function get(array $vars) {
		...
		$this->insert(array('dataset' => array('gen_info' => null, 'stat' => null)), $this->getContainer());
		...
		$options = array("gen_info", "stat", "permissions", "limits", "ippool", "subscriptions");
	...
	private function buildFilter(array $vars) {
		...
		if (isset($vars['id']))
			$this->insert(array('id' => $vars['id']), $filter_path);
		elseif (isset($vars['login']))
			$this->insert(array('login' => $vars['login']), $filter_path);
		elseif (isset($vars['guid']))
			$this->insert(array('guid' => $vars['guid']), $filter_path);
		elseif (isset($vars['all']) && $vars['all'])
			$this->insert(array('all' => null), $filter_path);
		elseif (isset($vars['external_id']))
			$this->insert(array('external-id' => $vars['external_id']), $filter_path);
	}
