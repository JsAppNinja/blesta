...
class PleskSubscriptions extends PleskPacket {
	...
	protected function buildContainer() {
		$this->insert(array('webspace' => null));
	 ...
	 * 		- htype The hosting type (e.g. virtual hosting, standard forwarding, frame forwarding, or none) (optional, default vrt_hst, one of:)
	 * 			- vrt_hst
	 * 			- std_fwd
	 * 			- frm_fwd
	 * 			- none
	 ...
	 * 		- ipv6 The IPv6 address (optional, required if ipv4 is not given) - only available with API RPC v1.6.3.2 or greater
	 * 	- plan An array of plan information (all optional) including:
	 * 		- id The ID of the service plan if necessary to create the subscription to a service plan (optional, takes precedence over plan_name if both are given)
	 * 		- name The service plan name if it is necessary to create a subscription to a certain service plan (optional)
	 * 		- guid The service plan GUID if it is necessary to create a subscription to a certain service plan (optional)
	...
	public function add(array $vars) {
		...
		$this->insert(array('add' => null), $this->getContainer());
		$this->setContainer($this->base_container . "/add");
		
		// Build gen_setup section
		$general = (isset($vars['general']) ? $vars['general'] : array());
		...
		if (!(isset($vars['general']['htype']) && $vars['general']['htype'] == "none")) {
			...
			$hosting = array_merge((isset($vars['hosting']) ? $vars['hosting'] : array()), array('htype' => $htype));
		...
		if (isset($vars['plan']['id']))
			$this->insert(array('plan-id' => $vars['plan']['id']), $this->getContainer());
		elseif (isset($vars['plan']['name']))
			$this->insert(array('plan-name' => $vars['plan']['name']), $this->getContainer());
		
		if (isset($vars['plan']['guid']))
			$this->insert(array('plan-guid' => $vars['plan']['guid']), $this->getContainer());
		if (isset($vars['plan']['external_id']))
			$this->insert(array('plan-external-id' => $vars['plan']['external_id']), $this->getContainer());
	...
	public function delete(array $vars) {
		// Set the container for this API request
		$this->insert(array('del' => null), $this->getContainer());
		$this->setContainer($this->base_container . "/del");
		
		$ids = (isset($vars['ids']) ? $vars['ids'] : array());
		$names = (isset($vars['names']) ? $vars['names'] : array());
		
		// Add the subscription IDs or names to delete
		$this->insert(array('filter' => null), $this->getContainer());
		if (!empty($ids)) {
			foreach ($ids as $id)
				$this->insert(array('id' => $id), $this->getContainer() . "/filter");
		}
		elseif (!empty($names)) {
			foreach ($names as $name)
				$this->insert(array('name' => $name), $this->getContainer() . "/filter");
		}
	 ...
	 * An owner MUST be specified unless the API version is 1.6.3.4 or greater. 1.6.3.4 allows no owner to be specified, which will receive plans from everyone
	 *
	...
	public function get(array $vars=array()) {
		// Set the container for this API request
		$this->insert(array('get' => null), $this->getContainer());
		...
		$this->insert(array('dataset' => null), $this->getContainer());
		
		// Always fetch general information
		$vars['settings'] = (isset($vars['settings']) ? $vars['settings'] : array());
		$vars['settings'] = array_merge($vars['settings'], array('gen_info' => true));
		...
		foreach ($options as $option) {
			if (array_key_exists($option, $vars['settings']) && $vars['settings'][$option])
				$this->insert(array($option => null), $this->getContainer() . "/dataset");
		}
	 ...
	 * 		- owner_id The ID of the owner/reseller that the subscription belong to, or that you wish to fetch (optional)
	 * 		- owner_login The login name of the owner/reseller that the subscription belong to, or that you wish to fetch (optional)
	 ...
	 * 		- admin_as_vendor True to set whether the administrator is the provider for the subscription, false otherwise (optional, default false)
	 ...
	 * 		- properties A key/value array of each property name and its value (optional)
	 * 		- dest_url The URL to which the user will be redirected implicitly at the attempt to visit the specified site (optional, required if htype is 'std_fwd' or 'frm_fwd')
	...
	public function set(array $vars) {
		// Set the container for this API request
		$this->insert(array('set' => null), $this->getContainer());
		$this->setContainer($this->base_container . "/set");
		
		// Build the filter section
		$filter = (isset($vars['filter']) ? $vars['filter'] : array());
		$this->buildFilter($filter);
		
		// Build the values container
		$this->insert(array('values' => null), $this->getContainer());
		
		// Build gen_setup section in the values section
		$general = (isset($vars['general']) ? $vars['general'] : array());
		$general = array_merge($general, (isset($vars['hosting']['htype']) ? array('htype' => $vars['hosting']['htype']) : array()));
		...
		if (!empty($vars['hosting']) && isset($vars['hosting']['htype']) && in_array($vars['hosting']['htype'], array("vrt_hst", "std_fwd", "frm_fwd")))
	...
	public function addPlan(array $vars) {
		// Set the container for this API request
		$this->insert(array('add-plan-item' => null), $this->getContainer());
		...
		$this->insert(array('plan-item' => null), $this->getContainer());
		
		// Add the plan to the plan-item section
		if (!empty($vars['plan']))
			$this->insert(array('name' => $vars['plan']), $this->getContainer() . "/plan-item");
	...
	public function removePlan(array $vars) {
		// Set the container for this API request
		$this->insert(array('remove-plan-item' => null), $this->getContainer());
	...
	public function changePlan(array $vars) {
		// Set the container for this API request
		$this->insert(array('switch-subscription' => null), $this->getContainer());
		...
		if (isset($vars['plan'])) {
			...
			if (isset($vars['plan']['no_plan']) && $vars['plan']['no_plan'])
				$this->insert(array('no-plan' => null), $this->getContainer());
		}
	 ...
	 * @param string $container The XML container to set the hosting section into (optional, defaults to the current container)
	 */
	private function buildHosting(array $vars, $container) {
		$container = (!empty($container) ? $container : $this->getContainer());
		
		$this->insert(array('hosting' => null), $container);
		
		// Build properties
		if ($vars['htype'] == "vrt_hst") {
			// Set the vrt_host section
			$this->insert(array('vrt_hst' => null), $container . "/hosting");
			
			// Add properties
			if (isset($vars['properties'])) {
				foreach ($vars['properties'] as $key => $value) {
					$property = array('property' => array('name' => $key, 'value' => $value));
					$this->insert($property, $container . "/hosting/vrt_hst");
				}
			}
		}
		elseif (in_array($vars['htype'], array("std_fwd", "frm_fwd"))) {
			// Set the type and dest_url
			$this->insert(array($vars['htype'] => array('dest_url' => (isset($vars['dest_url']) ? $vars['dest_url'] : ""))), $container . "/hosting");
		}
		else {
			// None
			$this->insert(array('none' => null), $container . "/hosting");
		}
		
		// Build IP addresses
		if (isset($vars['ipv4']))
			$this->insert(array('ip_address' => $vars['ipv4']), $container . "/hosting/" . $vars['htype']);
		if (isset($vars['ipv6']))
			$this->insert(array('ip_address' => $vars['ipv6']), $container . "/hosting/" . $vars['htype']);
	}
	 ...
	 * 	- owner_guid The Plesk user GUID (optional)
	 * 	- owner_external_id The ID of a Panel user in other components or applications (optional)
	 * 	- htype The hosting type (e.g. virtual hosting, standard forwarding, frame forwarding, or none) (optional, only for "add" type, default vrt_hst, one of:)
	 * 		- vrt_hst
	 * 		- std_fwd
	 * 		- frm_fwd
	 * 		- none
	 ...
	 * 		- 64 Disabled by a customer
	 * 	- external_id Specifies a GUID of a subscription owner received from the Panel components (optional)
	 * 	- admin_as_vendor True to set whether the administrator is the provider for the subscription, false otherwise (optional, only for the "set" type)
	...
	private function buildGenSetup(array $vars, $type="add") {
		...
		if ($type == "add") {
			...
			// Set hosting type
			if (isset($vars['htype']) && in_array($vars['htype'], array("vrt_hst", "std_fwd", "frm_fwd", "none")))
		...
		if (isset($vars['status']) && in_array($vars['status'], array("0", "16", "32", "64")))
	 ...
	 * 	- external_id Specifies a GUID of a subscription in the Panel component (optional)
	 */
	private function buildFilter(array $vars=array()) {
