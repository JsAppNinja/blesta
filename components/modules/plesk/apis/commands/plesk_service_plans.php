...
class PleskServicePlans extends PleskPacket {
	...
	protected function buildContainer() {
		$this->insert(array('service-plan' => null));
	...
	public function get(array $vars=array()) {
		...
		if (!empty($vars['plans'])) {
			foreach ($vars['plans'] as $plan) {
				$this->insert(array('name' => $plan), $this->getContainer() . "/filter");
			}
		}
		
		// Set the owner/reseller, if any
		if (isset($vars['owner_all']) && $vars['owner_all'])
			$this->insert(array('owner-all' => null), $this->getContainer());
		elseif (!empty($vars['owner_id']))
			$this->insert(array('owner-id' => $vars['owner_id']), $this->getContainer());
		elseif (!empty($vars['owner_login']))
			$this->insert(array('owner-login' => $vars['owner_login']), $this->getContainer());
