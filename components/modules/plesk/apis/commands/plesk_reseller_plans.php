...
class PleskResellerPlans extends PleskPacket {
	...
	protected function buildContainer() {
		$this->insert(array('reseller-plan' => null));
	 ...
	 * 	- owner_id The ID of the owner/reseller that the plans belong to, or that you wish to fetch (optional, will be used if owner_login is also specified)
	 * 	- owner_login The login name of the owner/reseller that the plans belong to, or that you wish to fetch (optional)
	 * 	- owner_all Set to true to fetch plans for all owners/resellers (optional, requires version 1.6.3.4; will be used if specified)
	...
	public function get(array $vars=array()) {
		...
		if (isset($settings['limits']) && $settings['limits'])
			$this->insert(array('limits' => null), $this->getContainer());
		if (isset($settings['permissions']) && $settings['permissions'])
			$this->insert(array('permissions' => null), $this->getContainer());
		if (isset($settings['ip_pool']) && $settings['ip_pool'])
			$this->insert(array('ip-pool' => null), $this->getContainer());
	 ...
	 * 	- guid The reseller plan GUID
	 * 	- external_id The reseller plan ID in the Panel components
	 * @param string $container The container path to insert this filter section into (optional, default to the base container)
	 */
	private function buildFilter(array $vars, $container=null) {
		...
		$this->insert(array('filter' => null), $container);
		...
		elseif (isset($vars['name']))
			$this->insert(array('name' => $vars['name']), $filter_path);
