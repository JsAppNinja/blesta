...
class Solusvm extends Module {
	...
	public function install() {
		...
		if (!extension_loaded("simplexml"))
			$errors['simplexml'] = array('required' => Language::_("SolusVM.!error.simplexml_required", true));
	 ...
	 * @param stdClass $package A stdClass object representing the selected package
	 * @param array $vars An array of user supplied info to satisfy the request
	 * @param boolean $edit True to validate editing a service, or false to validate adding one (optional, default false)
	 * @param stdClass $service_fields An stdClass object representing each service fields (optional)
	 * @return boolean True if the service validates, false otherwise. Sets Input errors when false.
	 */
	public function validateService($package, array $vars=null, $edit=false, $service_fields = null) {
		// Set rules
		$rules = array(
			'solusvm_hostname' => array(
				'format' => array(
					'pre_format' => array(array($this, "replaceText"), "", "/^\s*www\./i"),
					'rule' => array(array($this, "validateHostName"), true),
					'message' => Language::_("Solusvm.!error.solusvm_hostname.format", true)
				)
			),
			'solusvm_vserver_id' => array(
				'format' => array(
					'if_set' => true,
		...
		if (empty($vars['solusvm_vserver_id']) && !$edit)
		...
		if ($edit) {
			...
			if (isset($vars['use_module']) && $vars['use_module'] == "true" && $service_fields) {
				...
				if (!empty($service_fields->solusvm_extra_ip_addresses)) {
					...
					$remove_ips = (isset($vars['solusvm_remove_extra_ips']) ? (array)$vars['solusvm_remove_extra_ips'] : array());

					$rules['configoptions[customextraip]'] = array(
						'valid' => array(
							'if_set' => true,
							'rule' => array(array($this, "validateRemovingExtraIps"), $extra_ips, $remove_ips),
	...
	public function addService($package, array $vars=null, $parent_package=null, $parent_service=null, $status="pending") {
		...
		if (isset($vars['solusvm_hostname'])) {
			$vars['solusvm_hostname'] = $this->replaceText($vars['solusvm_hostname'], "", "/^\s*www\./i");
		...
		if ($vars['use_module'] == "true") {
			...
			$api_fields = array("type", "node", "nodegroup", "hostname", "password", "username", "plan", "template",
		...
		$fields = array(
			array(
				...
				'value' => (isset($response->vserverid) ? $response->vserverid : (!empty($vars['solusvm_vserver_id']) ? $vars['solusvm_vserver_id'] : null)),
				'encrypted' => 0
			),
			array(
	...
	public function editService($package, $service, array $vars=array(), $parent_package=null, $parent_service=null) {
		...
		$vars['configoptions'] = array_merge($vars['configoptions'], $ServiceOptions->getAll($vars['configoptions'], $plan));

		// Validate the service-specific fields
		$this->validateService($package, $vars, true, $service_fields);
		...
		if ($vars['use_module'] == "true") {
			...
			if (isset($delta['solusvm_template']) && isset($vars['confirm_reinstall']) && $vars['confirm_reinstall']) {
			...
			if (isset($vars['configoptions'])) {
				...
				$service_fields->solusvm_extra_ip_addresses = implode(",", $this->getExtraIps($service_fields, $row));
			}
		}
		...
		foreach ($service_fields as $key => $value)
			$fields[] = array('key' => $key, 'value' => $value, 'encrypted' => (in_array($key, $this->encrypted_fields) ? 1 : 0));
		...
		if (($row = $this->getModuleRow())) {
			...
			if ($package_from->meta->plan != $package_to->meta->plan) {
				...
				try {
					...
					$params = array('vserverid' => $service_fields->solusvm_vserver_id, 'plan' => $plan);
	...
	public function getAdminAddFields($package, $vars=null) {
		...
		$vserver_id = $fields->label(Language::_("Solusvm.service_field.solusvm_vserver_id", true), "solusvm_vserver_id");
		// Create virtual server field and attach to virtual server label
		$vserver_id->attach($fields->fieldText("solusvm_vserver_id", $this->Html->ifSet($vars->solusvm_vserver_id), array('id'=>"solusvm_vserver_id")));
		// Add tooltip
	...
	public function getClientAddFields($package, $vars=null) {
		...
		$host_name = $fields->label(Language::_("Solusvm.service_field.solusvm_hostname", true), "solusvm_hostname");
		// Create hostname field and attach to hostname label
		$host_name->attach($fields->fieldText("solusvm_hostname", $this->Html->ifSet($vars->solusvm_hostname, $this->Html->ifSet($vars->domain)), array('id'=>"solusvm_hostname")));
		// Set the label as a field
		$fields->setField($host_name);
	...
	public function getClientTabs($package) {
		return array(
			'tabClientActions' => array('name' => Language::_("Solusvm.tab_actions", true), 'icon' => "fa fa-gears"),
			'tabClientStats' => array('name' => Language::_("Solusvm.tab_stats", true), 'icon' => "fa fa-bar-chart"),
			'tabClientConsole' => array('name' => Language::_("Solusvm.tab_console", true), 'icon' => "fa fa-terminal"),
			'tabClientIps' => array('name' => Language::_("Solusvm.tab_ips", true), 'icon' => "fa fa-cog")
		);
	}
	...
	private function ipsTab($package, $service, $client=false, array $get=null, array $post=null) {
		...
		if (!empty($post['ip_address']) && in_array($post['ip_address'], $extra_ips) && $option_editable) {
			...
			$vars = array('solusvm_remove_extra_ips' => array($post['ip_address']));
			...
			$this->Services->edit($service->id, array_merge($vars, array('configoptions' => $options, 'prorate' => true)));
				...
				case "reinstall":
					...
					if (!empty($post)) {
						...
						if ($this->Input->validates($post)) {
							...
							Loader::loadModels($this, array("Services"));
							$this->Services->edit($service->id, array('solusvm_template' => $post['template'], 'confirm_reinstall' => true));
	...
	private function getExtraIps($service_fields, $module_row) {
		...
		$ips = $service_fields->solusvm_extra_ip_addresses;
		$ips = explode(",", $ips);
	...
	private function getTemplates($type, $module_row) {
		...
		try {
			...
			$params = array('type' => $type, 'listpipefriendly' => "true");
		...
		if ($response && $response->status == "success") {
			...
			switch (strtolower($type)) {
				case "kvm":
					$templates = array_merge((!empty($response->templateskvm) ? $this->csvToArray($response->templateskvm) : array()), $templates);
					break;
				case "xen hvm":
					$templates = array_merge((!empty($response->templateshvm) ? $this->csvToArray($response->templateshvm) : array()), $templates);
					break;
				default:
					break;
			}
	...
	private function getFieldsFromInput(array $vars, $package) {
		...
		// Set any config options
		$config_options = (isset($vars['configoptions']) ? (array)$vars['configoptions'] : array());
	 ...
	 * @param boolean $require_fqdn True to require a FQDN (e.g. host.domain.com), or false for a partial name (e.g. domain.com) (optional, default false)
	 ...
	 * @return boolean True if the number of IPs can be decreased, or false otherwise
	 */
