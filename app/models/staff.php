...
class Staff extends AppModel {
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			// Add staff
			$fields = array("user_id", "first_name", "last_name", "email", "email_mobile", "status");
	...
	public function edit($staff_id, array $vars) {
		$this->Input->setRules($this->getEditRules($vars));
		...
		if ($this->Input->validates($vars)) {
			...
			// Update staff
			$fields = array("first_name", "last_name", "email", "email_mobile", "status");
	...
	public function delete($staff_id) {
		...
		$this->Record->from("staff")->from("staff_group")->from("staff_settings")->
			where("staff.id", "=", $staff_id)->where("staff.id", "=", "staff_group.staff_id", false)->
			where("staff_group.staff_id", "=", "staff_settings.staff_id", false)->delete(array("staff.*", "staff_group.*", "staff_settings.*"));
	}
	...
	public function assignGroup($staff_id, $staff_group_id) {
		...
		$this->Record->from("staff_group")->
			...
			where("staff_group_id", "=", $staff_group_id)->delete();
			
		// Assign the staff group
		$this->Record->set("staff_id", $staff_id)->set("staff_group_id", $staff_group_id)->insert("staff_group");
	}
	...
	public function addNotice(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->duplicate("action", "=", $vars['action'])->
				insert("staff_notices", $vars, array("staff_group_id", "staff_id", "action"));
		}
	}
	 ...
	 * @param array $vars A list of staff notices, each containing:
	 *  - action The email group action
	 */
	public function addNotices($staff_id, $staff_group_id, array $actions) {
		...
		$rules['actions[]'] = array(
			'exists' => array(
				'if_set' => true,
				'rule' => array(array($this, "validateNoticeActionExists"), $staff_group_id),
		...
		if ($this->Input->validates($vars)) {
			...
			foreach ($vars['actions'] as $action) {
				$notice = array('staff_id'=>$staff_id, 'staff_group_id'=>$staff_group_id, 'action'=>$action);
				$this->Record->duplicate("action", "=", $action)->
					insert("staff_notices", $notice, array("staff_group_id", "staff_id", "action"));
			}
		}
	}
	...
	public function get($staff_id, $company_id=null) {
		$fields = array("staff.id", "staff.user_id", "staff.first_name", "staff.last_name",
			"staff.email", "staff.email_mobile", "staff.status", "users.username",
		...
		if ($staff) {
			...
			$this->Record->select(array("staff_groups.id", "staff_groups.company_id", "staff_groups.name"))->
				from("staff")->
	...
	public function getByUserId($user_id, $company_id=null) {
		...
		// Assign object properties only if staff exists
		if ($staff) {
			$staff->groups = $this->Record->select(array("staff_groups.id", "staff_groups.company_id", "staff_groups.name"))->
				from("staff")->innerJoin("staff_group", "staff_group.staff_id", "=", "staff.id", false)->
				innerJoin("staff_groups", "staff_group.staff_group_id", "=", "staff_groups.id", false)->
				where("staff.id", "=", $staff->id)->fetchAll();
	 ...
	 * 	- to To notice types
	 * @param string $status The status of the staff member ("active", "inactive", or null for all; optional, default null)
	...
	public function getAllByEmailAction($action, $company_id, $notice_type="all", $status=null) {
		...
		$this->Record->select(array("staff.*"))->from("staff")->
			innerJoin(array($staff_id_sql=>"temp"), "temp.staff_id", "=", "staff.id", false);
	 ...
	 */
	public function getList($company_id=null, $status=null, $page=1, $order_by=array('id'=>"ASC")) {
	...
	public function getAll($company_id=null, $status=null, $staff_group_id=null) {
		return $this->getStaff($company_id, $status, $staff_group_id)->
			order(array('first_name' => "asc"))->fetchAll();
	}
	 ...
	 * @param boolean $group_by_action True to group by the action, false otherwise (optional, default true)
	...
	public function addQuickLink($staff_id, $company_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			$fields = array("staff_id", "company_id", "uri", "title", "order");
	...
	public function getQuickLinks($staff_id, $company_id) {
		return $this->Record->select()->from("staff_links")->where("staff_id", "=", $staff_id)->
			where("company_id", "=", $company_id)->order(array("order"=>"ASC"))->fetchAll();
	}
	 ...
	 * @param array $tags An array of replacement tags containing the key/value pairs for the replacements where the key is the tag to replace and the value is the value to replace it with (optional; A 'staff' tag will automatically be set)
	 ...
	 */
	public function sendNotificationEmail($action, $company_id, array $tags=null, $cc=null, $bcc=null, array $attachments=null, array $options=null) {
		// Load Emails model
		...
		foreach ($staff as $staff_member) {
			...
			if (empty($tags))
				$tags = array();
			$tags = array_merge($tags, array('staff' => $staff_member));
			
			// Send the email
			$this->Emails->send($action, $company_id, $lang, $staff_member->email, $tags, $cc, $bcc, $attachments, $options);
		}
	}
	...
	public function getSettings($staff_id, $company_id) {
		// Staff Settings
		$sql1 = $this->Record->select(array("key", "value"))->from("staff_settings")->
		...
		// Company Settings
		$sql2 = $this->Record->select(array("key", "value"))->from("staff")->
			innerJoin("staff_group", "staff_group.staff_id", "=", "staff.id", false)->
			innerJoin("staff_groups", "staff_groups.id", "=", "staff_group.staff_group_id", false)->
		...
		// System settings
		$sql3 = $this->Record->select(array("key", "value"))->from("settings")->get();
		$values = $this->Record->values;
		$this->Record->reset();
		$this->Record->values = $values;
		
		return $this->Record->select()->from(array("((" . $sql1 . ") UNION (" . $sql2 . ") UNION (" . $sql3 . "))"=>"temp"))->group("temp.key")->fetchAll();
	}
	...
	public function getSetting($staff_id, $key, $company_id=null) {
		$sql = array();
		// Staff Settings
		$sql[] = $this->Record->select(array("key", "value"))->from("staff_settings")->
		...
		// Company Settings
		if ($company_id !== null) {
			$sql[] = $this->Record->select(array("key", "value"))->from("staff")->
				innerJoin("staff_group", "staff_group.staff_id", "=", "staff.id", false)->
				innerJoin("staff_groups", "staff_groups.id", "=", "staff_group.staff_group_id", false)->
				innerJoin("company_settings", "company_settings.company_id", "=", "staff_groups.company_id", false)->
				where("staff.id", "=", $staff_id)->where("company_settings.key", "=", $key)->where("staff_groups.company_id", "=", $company_id)->get();
			$values = $this->Record->values;
			$this->Record->reset();
			$this->Record->values = $values;
		}
		...
		// System settings
		$sql[] = $this->Record->select(array("key", "value"))->from("settings")->where("key", "=", $key)->get();
		$values = $this->Record->values;
		$this->Record->reset();
		$this->Record->values = $values;
		
		return $this->Record->select()->from(array("((" . implode(") UNION (", $sql) . "))"=>"temp"))->group("temp.key")->fetch();
		//return $this->Record->select()->from(array("((" . $sql1 . ") UNION (" . $sql2 . ") UNION (" . $sql3 . "))"=>"temp"))->group("temp.key")->fetch();
	}
	...
	public function setSetting($staff_id, $key, $value) {
		...
		$this->Record->duplicate("value", "=", $value)->
			insert("staff_settings", array("key"=>$key, "staff_id"=>$staff_id, "value"=>$value));
	}
	...
	private function getAddRules(array $vars) {
		$rules = array(
			'user_id' => array(
				...
				'unique' => array(
					'rule' => array(array($this, "validateExists"), "user_id", "staff"),
					'negate' => true,
					'message' => $this->_("Staff.!error.user_id.unique", $this->ifSet($vars['user_id']))
				)
			),
			'first_name' => array(
				'empty' => array(
			...
			'groups' => array(
				'unique_company' => array(
					'rule' => array(array($this, "validateUniqueCompanies"), $this->ifSet($vars['groups'])),
	 ...
	 * @return array The rules
	 */
	private function getEditRules(array $vars) {
	...
	private function getNoticeRules(array $vars) {
		$rules = array(
			...
			'action' => array(
				'exists' => array(
					'rule' => array(array($this, "validateNoticeActionExists"), $this->ifSet($vars['staff_group_id'])),
					'message' => $this->_("Staff.!error.action.exists", $this->ifSet($vars['action']))
				)
			)
		);
	 ...
	 * @return boolean True if none of the staff group IDs belong to the same company, false otherwise
	 */
	 ...
	 * @return boolean True if the staff group has the action available, false otherwise
	 */
