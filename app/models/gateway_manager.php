...
class GatewayManager extends AppModel {
	...
	public function getInstalled() {
		$fields = array("id", "company_id", "name", "class", "version", "type");
	...
	private function getGatewayInfo($gateway, $type, $company_id) {
		...
		$config = new stdClass();
		if (file_exists(COMPONENTDIR . "gateways" . DS . $type . DS . $class . DS . "config.json")) {
			$config = file_get_contents(COMPONENTDIR . "gateways" . DS . $type . DS . $class . DS . "config.json");
			if ($config) {
				if (!isset($this->Json))
