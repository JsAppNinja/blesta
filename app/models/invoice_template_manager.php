...
class InvoiceTemplateManager extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("invoice_template_manager"));
	}
	...
	public function getPdfFonts() {
		$dir = VENDORDIR . "tcpdf" . DS . "fonts" . DS;
		$fonts = array();
		$exclude_types = array("b", "i", "bi");
		...
		if ($dh = opendir($dir)) {
			while (($file = readdir($dh)) !== false) {
				...
				if (substr($file, -strlen($ext)) == $ext) {
					...
					if (in_array(substr($name, -2), $exclude_types) && file_exists($dir . substr($name, 0, -2) . $ext))
						continue;
					if (in_array(substr($name, -1), $exclude_types) && file_exists($dir . substr($name, 0, -1) . $ext))
						continue;
					$fonts[$name] = $name;
				}
			}
	...
	private function loadTemplate($class) {
		...
		if (!isset($this->InvoiceTemplates))
			Loader::loadComponents($this, array("InvoiceTemplates"));
