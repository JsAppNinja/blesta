...
class ModuleManager extends AppModel {
	...
	public function getInstalled() {
		...
		return $this->Record->select($fields)->from("modules")->fetchAll();
	}
	...
	public function delete($module_id) {
		...
		$rules = array(
			'module_id' => array(
				'assigned_package' => array(
					'rule' => array(array($this, "validateAssignedToPackage"), "!inactive"),
			...
			'module_row_id' => array(
				'assigned_service' => array(
					'rule' => array(array($this, "validateAssignedToService"), $module_id, "!canceled"),
		...
		$vars = array('module_id' => $module_id);
		if ($this->Input->validates($vars)) {
	...
	public function deleteRow($module_row_id) {
		...
		$rules = array(
			'module_row_id'=>array(
				...
				'assigned_service'=>array(
					'rule'=>array(array($this, "validateAssignedToService"), null, "!canceled"),
	 ...
	 * @param string The status type (null for all services, service type, or ! service type). For example:
	 * 	- active (only active service)
	 * 	- !active (only not active service)
	 * @return boolean True if the module_id is not assigned to any packages, false if it is
	 */
	public function validateAssignedToPackage($module_id, $status = null) {
		
		$this->Record->select(array("packages.id"))->from("packages")->
	...
	public function validateAssignedToService($module_row_id = null, $module_id = null, $status = null) {
		...
		$this->Record->select(array("services.id"))->from("services");
	...
	private function getModuleInfo($module, $company_id) {
		...
		$config = new stdClass();
		if (file_exists(COMPONENTDIR . "modules" . DS . $class . DS . "config.json")) {
			$config = file_get_contents(COMPONENTDIR . "modules" . DS . $class . DS . "config.json");
			if ($config) {
				if (!isset($this->Json))
