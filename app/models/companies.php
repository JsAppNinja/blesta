	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			try {
				...
				$signatures = $this->Record->select(array_merge(array("id"), $fields))->
				...
				$this->rollBack();
				$this->Record->reset();
				$this->Input->setErrors(array('exception' => array('message' => $e->getMessage())));
			}
			return $company_id;
		}
	}
		...
		if (is_dir($base_dir)) {
			if ($dh = opendir($base_dir)) {
				while (($directory = readdir($dh)) !== false) {
					...
					$view_dirs[$directory] = $this->Json->decode(file_get_contents($base_dir . DS . $directory . DS . "config.json"));
				}
