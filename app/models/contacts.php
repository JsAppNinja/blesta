...
class Contacts extends AppModel {
	...
	public function edit($contact_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$fields = array("user_id", "contact_type", "contact_type_id", "first_name", "last_name",
	...
	public function getPermissions($contact_id) {
		...
		return $this->Record->order(array('area' => "asc"))->
			fetchAll();
	}
	 ...
	 * @return boolean True if permission allowed, false otherwise
	 */
	...
	public function getPermissionOptions($company_id) {
		...
		foreach ($plugin_actions as $action) {
			$plugin_actions = $this->PluginManager->getActions($company_id, $action, true);
	...
	private function getAddRules(array $vars) {
		$rules = array(
			...
			'user_id' => array(
				'exists' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateExists"), "id", "users", false),
