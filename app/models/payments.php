...
class Payments extends AppModel {
	 ...
	 * @param array $invoice_amounts An array of key/value pairs where each key represents an invoice ID and each value is the amount to apply to that invoice
	...
	public function getBuildProcess(array $contact_info, $amount, $currency, array $invoice_amounts=null, array $options=null, $gateway_id=null) {
		...
		if ($invoice_amounts) {
			foreach ($invoice_amounts as $invoice_id => $amt)
				$inv_amounts[] = array('id' => $invoice_id, 'amount' => $amt);
		}
		if (isset($options['recur']['amount']))
			$options['recur']['amount'] = $this->currencyToDecimal($options['recur']['amount'], $currency);
		
		$result = $this->GatewayPayments->getBuildProcess($contact_info, $amount, $currency, $inv_amounts, $options, $gateway_id);
	...
	public function getBuildAuthorize(array $contact_info, $amount, $currency, array $invoice_amounts=null, array $options=null, $gateway_id=null) {
		...
		$result = $this->GatewayPayments->getBuildAuthorize($contact_info, $amount, $currency, $inv_amounts, $options, $gateway_id);
		if (($errors = $this->GatewayPayments->errors()))
			$this->Input->setErrors($errors);
		return $result;
	}
	 ...
	 * @return object An object representing the transaction if attempted, void otherwise. Check Payments::errors(), as some transactions may be attempted and yet still produce errors
	 */
	public function processPayment($client_id, $type, $amount, $currency, array $account_info=null, $account_id=null, array $options=null) {
		...
		$result = $this->GatewayPayments->processPayment($client_id, $type, $amount, $currency, $account_info, $account_id, $options);
		if (($errors = $this->GatewayPayments->errors()))
			$this->Input->setErrors($errors);
		return $result;
	}
	...
	public function authorizePayment($client_id, $type, $amount, $currency, array $account_info=null, $account_id=null, array $options=null) {
		if (isset($options['passphrase']))
			$this->GatewayPayments->setPassphrase($options['passphrase']);
		$result = $this->GatewayPayments->authorizePayment($client_id, $type, $amount, $currency, $account_info, $account_id, $options);
		if (($errors = $this->GatewayPayments->errors()))
			$this->Input->setErrors($errors);
		return $result;
	}
	 ...
	 * @return mixed An object representing the transaction if attempted, void otherwise. Check Payments::errors(), as some transactions may be attempted and yet still produce errors
	 */
