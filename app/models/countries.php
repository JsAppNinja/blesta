...
class Countries extends AppModel {
	...
	private function getRules(array $vars) {
		$rules = array(
			...
			'alpha3' => array(
				...
				'in_use' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateAlpha3InUse"), $this->ifSet($vars['alpha2'])),
					'negate' => true,
     ...
     * @param string $allow_alpha3 The ISO 3166-1 alpha3 country code to ignore if it happens to match $alpha3
     * @return boolean True if the alpha3 country code is already taken
     */
