...
class Permissions extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("permissions"));
	...
	public function get($id) {
		$fields = array("permissions.id", "permissions.group_id", "permissions.name",
		...
		if ($permission)
			$permission->name_trans = ($name = Language::_($permission->name, true)) != null ? $name : $permission->name;
		return $permission;
	}
	...
	public function getGroup($group_id) {
		$fields = array("permission_groups.id", "permission_groups.name", "permission_groups.level",
	 ...
	 * @return array An array of stdClass objects each representing a permission group. Each permission group also contains an array of permission fields representing as stdClass objects
	 */
	public function getAll($level, $company_id) {
		...
		$permissions = $this->Record->select($fields)->from("permission_groups")->
			...
			close()->
			where("permission_groups.level", "=", $level)->order(array('permission_groups.name'=>"asc"))->fetchAll();
			...
			foreach ($permissions as &$group) {
				...
				$group->permissions = $this->Record->select($fields)->from("permissions")->
					...
					open()->
						...
						orWhere("plugins.company_id", "=", $company_id)->
					close()->
					where("permissions.group_id", "=", $group->id)->order(array('permissions.name'=>"asc"))->fetchAll();
	 ...
	 * @return boolean True if the ARO is authorized to access the ACO or if the permission or permission group does not exist, false otherwise
	 */
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			$fields = array('group_id', 'name', 'alias', 'action', 'plugin_id');
	...
	public function delete($id) {
		...
		if ($permission->plugin_id) {
			$aros = $this->Record->select("acl_aro.alias")->from("staff_groups")->
				appendValues(array("staff_group_"))->
	...
	public function addGroup(array $vars) {
		...
		if ($this->Input->validates($vars)) {
			$fields = array('name', 'level', 'alias', 'plugin_id');
	 ...
	 *
	 * @param int $id The ID of the permission group to update
	 * @param array $vars An array of input fields including:
	 * 	- name The name of this permission group (language definition or plain-text)
	 * 	- level The level this permission group resides on (staff or client)
	 * 	- alias The ACO alias for this permission group (i.e. the Class name to apply to)
	...
	public function editGroup($group_id, array $vars) {
		...
		if ($this->Input->validates($vars)) {
			...
			$this->Record->where("id", "=", $id)->update("permission_groups", $vars, $fields);
	...
	private function getRules(array $vars) {
		$rules = array(
			'group_id'=>array(
				'exists'=>array(
					'rule'=>array(array($this, "validateExists"), "id", "permission_groups"),
