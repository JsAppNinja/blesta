...
(function($) {
    ...
    $.jqplot.PieRenderer.prototype.draw = function (ctx, gd, options, plot) {
		...
		if (this.showGradient) {
			if (this.gradientColor == undefined) {
				this.gradientColor = Array("rgba(255,255,255,0.45)", "rgba(255,255,255,0)");
			}
			
			var inner_r = parseInt(r*0.15); // inner is 15% size of outter
			var inner_offset = (this.gradientOffset == undefined || !this.gradientOffset ? 0 : inner_r);
			var inner_x = this._center[0]-inner_offset;
			var inner_y = this._center[1]-inner_offset;
			// inner circle (x, y, r), outter circle (x, y, r)
			var gradient = ctx.createRadialGradient(inner_x,inner_y,inner_r,this._center[0],this._center[1],r*(this.gradientSize == undefined ? 1 : this.gradientSize));
			gradient.addColorStop(0, this.gradientColor[0]);
			gradient.addColorStop(1, this.gradientColor[1]);
			ctx.fillStyle = gradient;
			ctx.arc(this._center[0],this._center[1], r, 0, 360, false);
			ctx.fill();
		}
    };
