...
class Api extends AppController {
	 ...
	 */
	private static $formats = array("json"=>"application/json", "php"=>"text/plain", "xml"=>"application/xml");
	/**
	...
	public function preAction() {
		...
		if (isset($this->get[0]) && isset($this->get[1])) {
			...
			$this->model_name = Loader::toCamelCase(strpos($this->model, ".") !== false ? ltrim(strrchr($this->model, "."), ".") : $this->model);
	...
	public function index() {
		...
		try {
			...
			$response = call_user_func_array(array($this->{$this->model_name}, $this->method), $params);
		}
		catch (Exception $e) {
	 ...
	 * @param boolean $encode True to automatically encode the given data, false otherwise.
	 */
	...
	private function encode($data) {
		switch ($this->format) {
			case "xml":
				if (isset($this->model))
					$data = array(strtolower($this->model) => $data);
	...
	private function authenticate() {
		...
		switch ($this->getAuthMode()) {
			...
			case "cli":
				$params = array();
				$this->post = array();
				$this->get = array($this->get[0], $this->get[1]);
				$this->put = array();
