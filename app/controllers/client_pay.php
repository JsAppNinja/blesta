...
class ClientPay extends AppController {
	...
	public function index() {
		...
		if ($show_past_due && count($vars->invoice_id) == 1 && $vars->invoice_id[0] == "all") {
			$vars->invoice_id = array();
		...
		$this->set("invoice_info", $this->partial("client_pay_multiple_invoices", array('vars'=>(isset($vars) ? $vars : new stdClass()), 'invoices'=>$invoice_list, 'toggle_amounts'=>$toggle_amounts, 'show_past_due' => $show_past_due)));
	}
	...
	public function method() {
		...
		if (isset($this->get[0]) && ($invoice = $this->Invoices->get($this->get[0])) && $invoice->client_id == $this->client->id) {
			...
			if (count($invoices['amounts']) === 1 && isset($invoices['amounts'][0]['invoice_id'])
            ...
            if (empty($errors) && isset($this->post['apply_credit']) && $this->post['apply_credit'] == "true") {
				...
				if ($temp_invoices != $invoices) {
					...
					if ($credit_covered_invoices && empty($invoices['credit'])) {
						...
						// Redirect to the login page if the client paid without being logged in, otherwise the client dashboard
		...
		$payment_types = array('' => Language::_("AppController.select.please", true));
		...
		$payment_accounts = array('' => Language::_("AppController.select.please", true));
		...
		$this->set("vars", $vars);
        $this->set("credits", array('currency' => $invoices['currency'], 'amount' => $total_credit));
     ...
     */
    private function applyCredit(array $invoices, array &$errors=array()) {
        // Nothing to apply a credit to
        if (empty($invoices['amounts']))
            return $invoices;
        
        $amounts_applied = $this->Transactions->applyFromCredits($this->client->id, $invoices['currency'], $invoices['amounts']);
