...
class AdminSystemCompanies extends AppController {
	...
	public function index() {
		...
		// Get all companies
		$companies = $this->Companies->getList($page, array($sort => $order));
	...
	public function add() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->Companies->errors())) {
				...
				$this->flashMessage("message", Language::_("AdminSystemCompanies.!success.company_added", true, $company->name));
				$this->redirect($this->base_uri . "settings/system/companies/");
			}
		}
		
		$this->set("companies", $this->Form->collapseObjectArray($this->Companies->getAllAvailable($this->Session->read("blesta_staff_id")), "name", "id"));
		$this->set("vars", $vars);
	}
	...
	public function edit() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->Companies->errors())) {
				...
				$this->flashMessage("message", Language::_("AdminSystemCompanies.!success.company_edited", true, $company->name));
				$this->redirect($this->base_uri . "settings/system/companies/");
			}
		}
	...
	public function delete() {
		...
		if (!empty($this->post)) {
			...
			if (($errors = $this->Companies->errors())) {
				...
				// Success
				$this->flashMessage("message", Language::_("AdminSystemCompanies.!success.company_deleted", true, $company->name));
			}
		}
		$this->redirect($this->base_uri . "settings/system/companies/");
	}
