...
class FeedReaderFeeds extends FeedReaderModel {
		...
		if ($this->Input->validates($vars)) {
			...
			if (isset($vars['company_id'])) {
				$this->Record->duplicate("feed_id", "=", "feed_id", false)->
					insert("feed_reader_defaults", array('feed_id'=>$feed_id,'company_id'=>$vars['company_id']));
			}
		}
	}
	...
	public function editFeed($feed_id, array $vars) {
		...
		if ($this->Input->validates($vars))
			$this->Record->where("id", "=", $feed_id)->update("feed_reader_feeds", $vars, array("updated"));			
	}
	...
	public function addArticle($feed_id, array $vars) {
		...
		$rules = array(
			'feed_id'=>array(
				'exists'=>array(
					'if_set'=>true,
					'rule'=>array(array($this, "validateExists"), "id", "feed_reader_feeds"),
		...
		if ($this->Input->validates($vars)) {
			$fields = array("feed_id", "guid", "data", "date");
	...
	public function addSubscriber($feed_id, $staff_id, $company_id) {
		...
		if ($this->Input->validates($vars)) {
			$fields = array('feed_id', 'company_id', 'staff_id');
	...
	public function deleteSubscriber($feed_id, $staff_id, $company_id) {
		...
		// If the feed is not a default feed then attempt to delete the feed (which only deletes if there are no more subscribers)
	 ...
	 * @param array $order  The order to list the feeds in key/value pairs where each key is a field and each value is a direction (asc or desc)
	 */
	public function getSubscribedFeeds($staff_id, $company_id, $order=array('id'=>"asc")) {

		$feeds = $this->Record->select(array("feed_reader_feeds.*"))->
			from("feed_reader_feeds")->on("feed_reader_subscribers.staff_id", "=", $staff_id)->on("feed_reader_subscribers.company_id", "=", $company_id)->
	 ...
	 */
	public function getDefaultFeeds($company_id, $order=array('id'=>"asc")) {
		return $this->Record->select(array("feed_reader_feeds.*"))->from("feed_reader_feeds")->
	 ...
	 * @param int $feed_id The ID of the feed to fetch articles for (if not given will fetch articles for ALL subscribed feeds)
	 ...
	 */
	public function getArticles($staff_id, $company_id, $feed_id=null, $page=1, $order=array('date'=>"desc")) {
		
		$this->Record = $this->articles($staff_id, $company_id, $feed_id);
		$articles = $this->Record->order($order)->limit($this->getPerPage(), (max(1, $page) - 1)*$this->getPerPage())->fetchAll();
	...
	public function fetchArticles($feed_id, $query_rate="0 seconds") {
		...
		$this->editFeed($feed_id, array('updated'=>date("c")));
		
		// Remove old records for this feed
		$sub_query = $this->Record->select(array("id"))->from("feed_reader_articles")->where("feed_id", "=", $feed_id)->
			order(array('date'=>"desc"))->limit(self::$articles_per_feed)->get();
		$sub_query_values = $this->Record->values;
		$this->Record->reset();
		
		$query = $this->Record->appendValues($sub_query_values)->select()->from(array($sub_query=>"temp"))->get();
		$query_values = $this->Record->values;
		$this->Record->reset();
		
		$this->Record->appendValues($query_values)->from("feed_reader_articles")->where("feed_reader_articles.id", "notin", array($query), false)->where("feed_reader_articles.feed_id", "=", $feed_id)->delete();
	}
	...
	private function articles($staff_id, $company_id, $feed_id=null) {
		...
		$this->Record->select(array("feed_reader_articles.*"))->from("feed_reader_feeds")->
	 ...
	 * @return boolean True if there are subscribers, false otherwise
	 */
	private function hasSubscribers($feed_id, $company_id=null) {
		$this->Record->select(array("feed_id"))->from("feed_reader_subscribers")->
	...
	private function getFeedIdByUrl($url) {
		$feed = $this->Record->select(array("id"))->from("feed_reader_feeds")->
			where("url", "=", $url)->fetch();
