...
class AdminMain extends ReassignPricingController
{
    ...
    public function index()
    {
        ...
        // Fetch the services
        $services = $this->Services->getList($this->client->id, $status, $page, array($sort => $order));
        ...
        $this->setPagination($paginate_uri, $total_services, array('sort' => $sort, 'order' => $order));
    }
    ...
    public function service()
    {
        ...
        $this->uses(array("Coupons", "ReassignPricing.ReassignPricingServices"));
    ...
    {
        $this->uses(array("ReassignPricing.ReassignPricingPackages"));
        ...
        $compatible_packages = (empty($compatible_packages) ? array($service->package) : $compatible_packages);

        $terms = array();
        foreach ($compatible_packages as $package) {
            $terms['package_' . $package->id] = array('name' => $package->name, 'value' => "optgroup");
